<x-app-layout>
    <nav class="text-gray-600 text-sm mb-4">
        <a href="#" class="hover:underline">Inicio</a> / <a href="/dashboard/profesores"
            class="hover:underline">Profesores</a> / <span class="font-semibold">Packs</span>
    </nav>
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="px-5 pb-5">
                    ...
                    <form method="POST" class="space-y-4" id="formDatos" enctype="multipart/form-data">
                        ...
                        <div>
                            <label for="books">Seleccionar packs</label>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function saveData() {
                ...
                if (selectedBooks) {
                    selectedBooks.forEach(bookId => formData.append("packs[]", bookId));
                }


                // Definir si es un POST (nuevo) o PUT (editar)
                let method = "POST";
                let url =  `/dashboard/profesor/asignar-packs/${rowId}`;
            ...
            function showModal(title, row = null) {
                ...
                if (data && data.packs && Array.isArray(data.packs)) {
                    let selectedBooks = data.packs.map(book => book.id);
                    ...
                    // Esperamos a que Select2 termine de cargar y luego asignamos los packs seleccionados
                    setTimeout(() => {
                        selectedBooks.forEach(bookId => {
                            let book = data.packs.find(b => b.id === bookId);
                            if (book) {
                                let newOption = new Option(book.name, book.id, true, true);
